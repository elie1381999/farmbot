...
import logging
from farmcore import farm_core  # Changed import to farmcore
...
async def add_treatment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if farm_core is None:
        logger.error("FarmCore is not initialized in add_treatment function")
...
async def treatment_crop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_crop function")
        if update.callback_query:
            await update.callback_query.message.reply_text(
                "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
                "Server error, please try again later."
...
async def treatment_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_product function")
        await update.message.reply_text(
            "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
            "Server error, please try again later."
        )
...
async def treatment_date_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    data = query.data or ""
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_date_callback function")
...
async def treatment_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_date function")
        await update.message.reply_text(
            "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
            "Server error, please try again later."
        )
...
async def treatment_cost(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_cost function")
        await update.message.reply_text(
            "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
            "Server error, please try again later."
        )
...
async def treatment_skip_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    data = query.data or ""
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_skip_callback function")
        await query.message.reply_text(
            "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
            "Server error, please try again later."
        )
...
async def treatment_next_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    if farm_core is None:
        logger.error("FarmCore is not initialized in treatment_next_date function")
        await update.message.reply_text(
            "خطأ في الخادم، يرجى المحاولة لاحقًا." if context.user_data.get('language', 'ar') == 'ar' else
            "Server error, please try again later."
        )
